import { Button } from "@/components/ui/button";

export default function Hero() {
  return (
    <section className="border-b border-border px-6 pb-20 pt-20 md:pt-28">
      <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-[1fr_auto] md:items-end">
        <div>
          <p className="mb-6 text-sm font-medium text-primary">Synfolix / Technology &amp; Product</p>
          <h1 className="max-w-3xl text-[2.75rem] font-semibold leading-[1.05] tracking-tight text-foreground md:text-6xl">
            We build digital products that solve real business problems.
          </h1>
          <p className="mt-6 max-w-xl text-lg text-muted-foreground">
            From our own software products to custom platforms built for businesses, Synfolix
            designs, develops and scales digital solutions across industries.
          </p>
          <div className="mt-10 flex flex-wrap items-center gap-4">
            <Button asChild size="lg" className="rounded-sm px-6">
              <a href="#products">Explore our products</a>
            </Button>
            <Button asChild variant="outline" size="lg" className="rounded-sm px-6">
              <a href="#contact">Build with Synfolix</a>
            </Button>
          </div>
        </div>
        <dl className="grid grid-cols-3 gap-6 border-t border-border pt-6 text-sm md:w-72 md:grid-cols-1 md:border-l md:border-t-0 md:pl-8 md:pt-0">
          <div>
            <dt className="text-muted-foreground">Model</dt>
            <dd className="mt-1 font-medium text-foreground">Products &amp; custom builds</dd>
          </div>
          <div>
            <dt className="text-muted-foreground">Reach</dt>
            <dd className="mt-1 font-medium text-foreground">Multi-industry</dd>
          </div>
          <div>
            <dt className="text-muted-foreground">Focus</dt>
            <dd className="mt-1 font-medium text-foreground">Engineering-led</dd>
          </div>
        </dl>
      </div>
    </section>
  );
}
